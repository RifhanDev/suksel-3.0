...
class CodeRequest extends Model
{
	...
	public function processDistrict($data)
	{
		...
		if (isset($data['ssm_expiry'])) {
			$data['ssm_expiry'] = Carbon::parse($data['ssm_expiry'])->format('Y-m-d');
		}
	...
	public function updateData()
	{
		...
		if ($this->type == 'district') {
			...
			if (isset($this->data['ssm_expiry']) && $this->data['ssm_expiry'] != "") {
				$this->vendor->ssm_expiry = $this->data['ssm_expiry'];
			}
		}
