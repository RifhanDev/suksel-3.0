...
class CustomerQuestionController extends Controller
{
    ...
    {
        if (!CustomerQuestion::canList()) {
            return $this->_access_denied();
        }

        if ($request->ajax()) {

			$faq_log_list = CustomerQuestion::leftJoin('faq_categories', 'customer_questions.faq_category_id', '=', 'faq_categories.id')
                        ->select(DB::raw("customer_questions.*, faq_categories.name"));
            ...
            if ($orderByColumn == 2)
            {
                $faq_log_list->orderBy('customer_questions.question', $orderByDir);
            }
			...
			$faq_log_list->where( function($q) use($keyword){
				$q->whereRaw("lower(customer_questions.question) like ?", '%'.$keyword.'%')
			...
			foreach ($results as $rows) {
				...
				$actions    = [];
                $actions[] = CustomerQuestion::canShow() ? "<a class='btn btn-xs btn-primary' href='".route('chatbot-manager.chatlog.show', ['chatlog' => $enc_id])."'>Papar</a>" : '';
                $actions[] = CustomerQuestion::canUpdate() ? "<a class='btn btn-xs btn-warning' href='".route('chatbot-manager.chatlog.edit', ['chatlog' => $enc_id])."'>Kemaskini</a>" : '';
                $actions[] = CustomerQuestion::canDelete() ? "<button class='btn btn-xs btn-danger' onclick='popupDelete(".'"'.$enc_id.'"'.")'>Padam</button>" : '';
        ...
        return view('customer-question.index');
    }
}
