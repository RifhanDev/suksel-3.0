...
class PenyediaanSuratSstController extends Controller
{
    ...
    public function index()
    {
        $tenders = Tender::query()
            ->where('status_process_id', TenderProcessStatus::penyediaanSuratSstListStatus())
            ->orderByDesc('id')
            ->get()
            ->map(fn (Tender $tender) => $this->mapTenderAdvertRow($tender, 'penyediaanSST'))
            ->values()
            ->all();
    ...
    public function hantar(Request $request)
    {
        ...
        if (! $this->advanceTenderProcess(
            ...
            return response()->json([
                'message' => 'Tender belum sedia untuk penyediaan surat SST (status ' . TenderProcessStatus::penyediaanSuratSstListStatus() . ').',
            ], 422);
        }

        return response()->json(['message' => 'Penyediaan surat SST berjaya dihantar.']);
    }
