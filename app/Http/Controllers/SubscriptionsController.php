...
class SubscriptionsController extends Controller
{
	...
	public function receipt($parent_id, $id)
	{
		...
		// Allow Admin and Agency Admin to force receipt type via query parameter
		$requestType = request()->get('type');
		if (($requestType == 'ASAL' || $requestType == 'SALINAN')
			&& (auth()->user()->hasRole('Admin') || auth()->user()->hasRole('Agency Admin'))
		) {
			$type = $requestType;
		} else {
			// Default behavior: determine type based on receipt_generated_at
			$type = 'SALINAN';
