	...
	<div id="left-pane">
		...
		<div class="well">
				...
				<div class="btn-group">
					...
					@if ($vendor->canUpdate2())
						...
						<ul class="dropdown-menu pull-right">
							<li><a href="{{ action('VendorsController@editEmail', $vendor->id) }}" class="btn btn-default">Emel
									/ No. Pendaftaran</a></li>
						</ul>
					@endif
				</div>
			...
			@if ($vendor->canConfirm())
				<a href="{{ action('UsersController@resendConfirmation', $vendor->user->id) }}" class="btn btn-default">Hantar
					Emel Pengesahan</a>
			@endif

			@if ($vendor->canApprove())
				<a href="{{ action('VendorsController@approve', [$vendor->id]) }}" class="btn btn-primary link-confirm">Lulus</a>
			...
			@if (Auth::user()->can('Vendor:histories'))
				<div class="btn-group">
					...
					<ul class="dropdown-menu pull-right">
						<li><a href="{{ action('UsersController@histories', $vendor->user->id) }}" class="btn btn-default">Aktiviti
								Pengguna</a></li>
					</ul>
				</div>
			@endif
