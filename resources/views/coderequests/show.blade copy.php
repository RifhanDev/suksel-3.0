	...
	<div id="left-pane">
		...
		<table class="table table-bordered">
			...
			@if ($request->type == 'mof')
				...
				@if (isset($request->data['bumiputera_company']))
					<tr>
						...
						@if ($request->status == 'pending')
							<td style="border-color: #000000 !important;" class="col-lg-5"><?= boolean_icon($request->vendor->mof_bumi) ?>
							</td>
						@endif
			...
			@foreach ($request->files()->orderBy('label', 'desc')->get() as $file)
				<tr>
					...
					<td style="border-color: #000000 !important;" @if ($request->status == 'pending') colspan="2" @endif>
						<button class="btn btn-warning btn-file-view" data-url="{{ $file->url . '/' . $file->name }}">Lihat</button>
					</td>
				</tr>
			@endforeach
		</table>
		...
		<div class="well">
			@if ($request->canProcess())
				...
				@if (isset($vendor))
					{!! Former::open(route('vendor.requests.approve', ['vendor' => $vendor->id, 'requests' => $request->id]))->class(
					    'form-inline',
					) !!}
				@else
			...
			@if (App\CodeRequest::canList())
				<a
					href="{{ route(isset($vendor) ? 'vendor.requests.index' : 'requests.index', isset($vendor) ? $vendor->id : null) }}"
							...
							@foreach ($templates as $template)
								<div class="col-sm-12">
									<label class="checkbox-inline" data-html="true" data-toggle="tooltip" data-placement="right"
										...
										<input type="checkbox" id="cb{{ $template->id }}" name="template" value="{{ $template->id }}">
										{{ $template->title }}
									</label>
								</div>
							@endforeach
