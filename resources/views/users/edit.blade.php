...
@section('content')
	...
	<div class="well">
		...
		@if (Auth::user()->hasRole('Admin') && !$currentUser->confirmed)
			<a href="{{ action('UsersController@resendConfirmation', $currentUser->id) }}" class="btn btn-default">Hantar Emel
				Pengesahan</a>
		@endif

		@if ($currentUser->canSetPassword())
			<a href="{{ action('UsersController@getSetPassword', $currentUser->id) }}" class="btn btn-default">Tukar Kata
				Laluan</a>
		@endif
