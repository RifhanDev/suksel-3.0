...
@section('content')
	...
	<h2>
		Maklumat Transaksi: {{ $transaction->number }}</h2>
	<hr>
	...
	<table class="table table-bordered">
		...
		<tr>
			...
			<td>
				@if ($transaction->gateway_auth)
					{{ $transaction->gateway_auth }}
				@else
					<span class="glyphicon glyphicon-remove"></span>
				@endif
			</td>
		...
		<table class="table table-bordered">
			@if ($transaction->subscription)
				<tr>
					...
					<td>{{ \Carbon\Carbon::parse($transaction->subscription->start_date)->format('d/m/Y') }} -
						{{ \Carbon\Carbon::parse($transaction->subscription->end_date)->format('d/m/Y') }}</td>
				</tr>
				<tr>
					<th>Resit Pembayaran</th>
					<td>
						{{ link_to_route('vendors.subscriptions.receipt', 'Resit', [$transaction->vendor_id, $transaction->subscription->id], ['target' => 'new', 'class' => 'btn btn-xs btn-warning']) }}
					</td>
				</tr>
			@elseif($transaction->cached_subscription)
				<tr>
					<th class="col-lg-3">Tempoh Langganan</th>
					<td>{{ \Carbon\Carbon::parse($transaction->cached_subscription->start_date)->format('d/m/Y') }} -
						{{ \Carbon\Carbon::parse($transaction->cached_subscription->end_date)->format('d/m/Y') }}</td>
				</tr>
				@if ($transaction->status == 'success')
					<tr>
						<th>Resit Pembayaran</th>
						<td>
							{{ link_to_route('transactions.temp_receipt', 'Resit', $transaction->id, ['target' => 'new', 'class' => 'btn btn-xs btn-warning']) }}
						</td>
					</tr>
				@endif
			@else
					...
					@foreach ($transaction->cached_purchases as $tender)
						<tr>
							...
							<td>
								...
								@if ($transaction->status == 'success')
									<br><a href="{{ route('transactions.temp_receipt', [$transaction->id, 'tender_id' => $tender->id]) }}"
										target="_blank"><i class="icon-printer"> Resit</i></a><br>
								@endif
							</td>
						</tr>
					@endforeach
	...
	<div class="well">
		...
		@if ($transaction->gateway && $transaction->gateway->type == 'ebpg')
			<form method="post" id="transaction_query" class="form-inline" action="{{ $transaction->gateway->daemon_url }}">
				...
				<input type="hidden" name="MERCHANT_TRANID"
					value="{{ $transaction->gateway->transaction_prefix }}{{ $transaction->number }}">
