...
@section('content')
	...
	@if ($tender->publish_winner)
		<ul class="nav nav-pills">
			<li{{ !Request::get('show') ? ' class="active"' : '' }}><a
					href="{{ asset('tenders/' . $tender->id . '/vendors') }}">Carta Tender</a></li>
				<li{{ Request::get('show') == 'winner' ? ' class="active"' : '' }}><a
						href="{{ route('tenders.vendors', [$tender->id, 'show' => 'winner']) }}">Penender Berjaya</a></li>
		</ul>
	@endif
		...
		@if (isset($winner) && $tender->publish_winner)
			<table class="table table-bordered">
				...
				<tr>
					<th>Harga Tawaran</th>
					<td>
						{{ $winner->price ? 'RM ' . number_format($winner->price, 2) : '<span class="glyphicon glyphicon-remove"></span>' }}
					</td>
				</tr>
			</table>
		@else
