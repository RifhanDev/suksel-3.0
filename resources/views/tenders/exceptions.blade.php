						...
						<tr>
							...
							<td>
								@forelse ($exception->files as $key => $value)
									<button class="btn btn-warning btn-file-view" data-url="{{ $value->url . '/' . $value->name }}">Lihat</button>
								@empty
									Tiada Fail
								@endforelse
							</td>
							<td>
								@if ($exception->status == 0 && $exception->canApprove())
									...
									<button type="button" id="reject" data-id="{{ $exception->id }}" class="btn btn-danger">Tolak</button>
								@else
									-
								@endif
							</td>
