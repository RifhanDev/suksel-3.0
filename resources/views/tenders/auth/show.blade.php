	...
	<div class="row g-4">
		<div class="col-lg-9 col-12">
			...
			<div class="d-flex flex-column flex-lg-row justify-content-between align-items-start align-items-lg-center mb-4">
				<div class="mb-3 mb-lg-0">
					<p class="text-uppercase text-muted small fw-semibold mb-1">
						{{ App\Tender::$types[$tender->type] }} &bull; {{ $tender->ref_number }}
					</p>
					<h3 class="fw-bold text-dark m-0" style="letter-spacing: -0.3px;">
						{{ $tender->name }}
					</h3>
			...
			<div class="card border shadow-sm rounded-3 overflow-hidden">
				<div class="row g-0">
					<div class="col-lg-3 border-end bg-light-subtle hidden-print">
						<div class="nav flex-column nav-pills py-3" role="tablist">
							<a href="#tf-main" aria-controls="tf-main" role="tab" data-bs-toggle="tab"
								class="nav-link @if (!Session::get('ErrorRequest') and !isset($active_prestasi_tab)) active @endif">
								<i class="ti ti-info-circle me-2"></i>Maklumat {{ App\Tender::$types[$tender->type] }}
							</a>
							<a href="#tf-syarat" aria-controls="tf-syarat" role="tab" data-bs-toggle="tab" class="nav-link">
								<i class="ti ti-file-description me-2"></i>Syarat {{ App\Tender::$types[$tender->type] }}
							</a>
							{{-- @if (count($tender->siteVisits) > 0) --}}
							<a href="#tf-lawatan" aria-controls="tf-lawatan" role="tab" data-bs-toggle="tab" class="nav-link">
							...
							@if (count($tender->mof_codes) > 0 || count($tender->cidb_grades) > 0 || count($tender->cidb_codes) > 0)
								<a href="#tf-kod" aria-controls="tf-kod" role="tab" data-bs-toggle="tab"
							...
							@if (count($tender->table_files) > 0)
								<a href="#tf-doc1" aria-controls="tf-doc1" role="tab" data-bs-toggle="tab" class="nav-link">
							...
							@if (Auth::check() && $tender->canShowFiles(Auth::user()->vendor_id))
								<a href="#tf-doc2" aria-controls="tf-doc2" role="tab" data-bs-toggle="tab" class="nav-link">
							...
							@if (Auth::check() && $tender->canUpdate() && $tender->invitation)
								<a href="#tf-invites" aria-controls="tf-invites" role="tab" data-bs-toggle="tab" class="nav-link">
									<i class="ti ti-mail me-2"></i>Senarai Jemputan
								</a>
							@endif
							@if (Auth::check() && $tender->canUpdate())
								<a href="#tf-history" aria-controls="tf-history" role="tab" data-bs-toggle="tab" class="nav-link">
									<i class="ti ti-history me-2"></i>Sejarah Pengubahan
								</a>
							@endif
							<a href="#tf-news" aria-controls="tf-news" role="tab" data-bs-toggle="tab" class="nav-link">
							...
							@if (auth()->check())
								@if (
									...
									<a href="#tf-exception" aria-controls="tf-exception" role="tab" data-bs-toggle="tab" class="nav-link">
										<i class="ti ti-key me-2"></i>Kebenaran Khas
									</a>
								@endif
							@endif
							<a href="#tf-officer" aria-controls="tf-officer" role="tab" data-bs-toggle="tab" class="nav-link">
								<i class="ti ti-user me-2"></i>Pegawai Bertanggungjawab
							</a>
							{{-- Tab - Penilaian Prestasi Syarikat --}}
							@if ($tender_winner)
								<a href="#tf-penilaian-prestasi" aria-controls="tf-penilaian-prestasi" role="tab" data-bs-toggle="tab"
					...
					<div class="tab-content col-lg-9 p-4">
				...
				<tbody>
					...
					@endforelse
						...
						<div role="tabpanel" class="tab-pane" id="tf-lawatan">
							<div class="card border shadow-sm rounded-3">
								<div class="card-header bg-light py-3 border-bottom d-flex align-items-center gap-2 fw-bold text-secondary">
									...
									Senarai Lawatan Tapak / Taklimat
								</div>
									...
									<div class="table-responsive">
										<table class="table table-bordered mb-0">
											<thead class="bg-danger text-white">
												<tr>
													<th class="text-center" style="width: 6%;">Bil.</th>
													<th>No / Tajuk</th>
													<th>Lokasi / Alamat</th>
													<th class="text-center" style="width: 18%;">Tarikh &amp; Waktu</th>
													<th class="text-center" style="width: 10%;">Wajib Hadir</th>
													<th class="text-center" style="width: 14%;">Wakil Syarikat</th>
												</tr>
											</thead>
											<tbody>
												@if (count($tender->siteVisits) > 0)
													@foreach ($tender->siteVisits->sortBy('id') as $visit)
														<tr>
															...
															<td>
																<div class="fw-semibold">{{ $tender->ref_number }}</div>
																<div class="small text-muted">{{ $tender->name }}</div>
															</td>
															<td>
																@if ($visit->meetpoint)
																	<div class="fw-semibold mb-1">{!! nl2br($visit->meetpoint) !!}</div>
																@endif
																<div class="text-muted small">{!! nl2br($visit->address) !!}</div>
															</td>
															<td class="text-center">
																{{ Carbon\Carbon::parse($visit->datetime)->format('d M Y') }}<br>
																<small>{{ Carbon\Carbon::parse($visit->datetime)->format('H:i') }}</small>
															</td>
															<td class="text-center">
																@if ($visit->required)
															...
															<td class="text-center">
																<button type="button" class="btn btn-link p-0 me-2 text-danger btn-visit-reps-view"
																	data-bs-toggle="modal" data-bs-target="#wakilModal" data-visit-id="{{ $visit->id }}"
																	data-visit-title="{{ $tender->ref_number }} - {{ $tender->name }}">
																	<i class="ti ti-eye"></i>
																</button>
																<button type="button" class="btn btn-link p-0 text-danger btn-visit-reps-edit" data-bs-toggle="modal"
																	data-bs-target="#wakilModal" data-visit-id="{{ $visit->id }}"
																	data-visit-title="{{ $tender->ref_number }} - {{ $tender->name }}">
												...
												@else
													<tr>
														<td colspan="6" class="text-center">Tiada lawatan tapak.</td>
													</tr>
												@endif
											</tbody>
										</table>
						...
						{{-- Modal: Wakil Syarikat untuk Lawatan Tapak --}}
						<div class="modal fade" id="wakilModal" tabindex="-1" aria-hidden="true">
							<div class="modal-dialog modal-lg modal-dialog-centered">
								<div class="modal-content">
									<div class="modal-header bg-danger text-white">
										<h6 class="modal-title">
											Wakil Syarikat &ndash; <span id="wakilModalTitle"></span>
										</h6>
									...
									<div class="modal-body">
										<form id="wakilForm">
											<input type="hidden" id="wakilVisitId" name="visit_id" value="">
											<div class="table-responsive">
												<table class="table table-bordered align-middle mb-3">
													<thead>
														<tr>
															<th style="width: 25%;">No IC</th>
															<th>Nama Individu</th>
															<th class="text-center" style="width: 15%;">Kehadiran</th>
														</tr>
													</thead>
													<tbody>
														@for ($i = 1; $i <= 3; $i++)
															<tr>
																<td>
																	<input type="text" class="form-control"
																		name="reps[{{ $i }}][ic_no]"
																		placeholder="Contoh: 900101011111"
																		id="wakil_ic_{{ $i }}">
																</td>
																<td>
																	<input type="text" class="form-control"
									...
									<div class="modal-footer justify-content-center">
										...
										<button type="button" class="btn btn-success px-4" id="wakilSaveBtn">
											Simpan
										</button>
									</div>
								</div>
	...
	@section('scripts')
		...
		<script>
			document.addEventListener('DOMContentLoaded', function() {
				var modalTitle = document.getElementById('wakilModalTitle');
				var wakilModal = document.getElementById('wakilModal');
			var visitIdInput = document.getElementById('wakilVisitId');
			var saveBtn = document.getElementById('wakilSaveBtn');
			var form = document.getElementById('wakilForm');

				if (!wakilModal) return;

				wakilModal.addEventListener('show.bs.modal', function(event) {
					var button = event.relatedTarget;
					if (!button) return;
					var title = button.getAttribute('data-visit-title') || '';
				...
				for (var i = 1; i <= 3; i++) {
					var ic = document.getElementById('wakil_ic_' + i);
					var nama = document.getElementById('wakil_nama_' + i);
					var hadir = document.getElementById('wakil_hadir_' + i);
					if (ic) ic.value = '';
					if (nama) nama.value = '';
					if (hadir) hadir.checked = false;
				}

				// Fetch existing representatives for this visit (if any)
				if (visitId) {
					fetch('/visits/' + visitId + '/representatives', {
						...
						.then(function(data) {
							...
							data.slice(0, 3).forEach(function(rep, index) {
								var i = index + 1;
								var ic = document.getElementById('wakil_ic_' + i);
								var nama = document.getElementById('wakil_nama_' + i);
								var hadir = document.getElementById('wakil_hadir_' + i);
								if (ic) ic.value = rep.ic_no || '';
								if (nama) nama.value = rep.name || '';
								if (hadir) hadir.checked = !!rep.attended;
							});
						})
						.catch(function() {
			...
			if (saveBtn && form) {
				saveBtn.addEventListener('click', function() {
					var visitId = visitIdInput ? visitIdInput.value : '';
					if (!visitId) return;

					var formData = new FormData(form);
					var tokenMeta = document.querySelector('meta[name="csrf-token"], meta[name="_token"]');
					...
					fetch('/visits/' + visitId + '/representatives', {
						...
						.then(function() {
							var modalInstance = bootstrap.Modal.getInstance(wakilModal);
							if (modalInstance) {
								modalInstance.hide();
							}
						})
