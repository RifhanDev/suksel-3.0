	...
	<div id="laporanArea">
		<div id="pageDetail">
					...
					<div class="row g-3 pb-3">
						...
						<div class="col-12 col-sm-12 col-lg-12">
							<div class="d-flex flex-column gap-1">
								<span class="text-muted fw-semibold text-uppercase" style="font-size:0.67rem; letter-spacing:0.5px;">Nama
									{{ @$tender->procurement_label }}</span>
						...
						<div class="col-12 col-sm-6 col-lg-4">
							<div class="d-flex flex-column gap-1">
								<span class="text-muted fw-semibold text-uppercase" style="font-size:0.67rem; letter-spacing:0.5px;">No.
									{{ @$tender->procurement_label }}</span>
								<span class="fw-semibold text-dark" style="font-size:0.88rem;">
					...
					<ul class="nav nav-tabs" id="committeeMainTabs" role="tablist">
						@foreach ($availableTabs as $index => $tab)
							<li class="nav-item" role="presentation">
								<button class="nav-link {{ $index === 0 ? 'active' : '' }}" id="tab-{{ $tab }}-btn"
									data-bs-toggle="tab" data-bs-target="#tab-{{ $tab }}" data-tab="{{ $tab }}" type="button"
									role="tab">
									{{ $tabLabels[$tab] ?? $tab }}
								</button>
							</li>
			...
			<div class="tab-content" id="committeeContent">
				...
				@foreach ($availableTabs as $index => $tab)
					<div id="tab-{{ $tab }}" class="tab-pane fade committee-pane {{ $index === 0 ? 'show active' : '' }}"
						data-jenis="{{ $tab }}" role="tabpanel">
							...
							<div class="content-card-header p-4 pb-3 border-bottom">
								<div class="d-flex align-items-center gap-3">
									...
									<div>
										...
										<p class="text-muted mb-0" style="font-size: 0.78rem;">Rekod catatan mesyuarat dan muat naik dokumen
											berkaitan</p>
									</div>
								</div>
							</div>
							<div class="content-card-body p-0">
								<div class="row g-0">
									...
									<div class="col-12 col-md-6 p-4" style="border-right: 1px solid #f1f5f9;">
										<p class="fw-semibold text-uppercase mb-3" style="font-size: 0.7rem; letter-spacing: 0.6px; color: #64748b;">
											Catatan</p>
										<textarea class="form-control committee-catatan" rows="5" placeholder="Masukkan catatan mesyuarat...">{{ $committeeDrafts[$tab]['catatan'] ?? '' }}</textarea>
									</div>
									...
									<div class="col-12 col-md-6 p-4">
										...
										<div class="mt-2 d-flex align-items-center gap-2 px-3 py-2 rounded-2 committee-file-name-wrapper"
											...
											<span class="small fw-semibold committee-file-name"
												style="color:#15803d;">{{ $committeeDrafts[$tab]['dokumen_sokongan_nama'] ?? '' }}</span>
										</div>
									</div>
	...
	<script>
				...
				render: {
					option: function(item, escape) {
						return '<div>' +
							...
							'<br><small class="text-muted">' + escape(item.name || '-') + ' | ' + escape(item
								.email || '-') + ' | ' + escape(item.jawatan || '-') + '</small>' +
							'</div>';
					},
		...
		function collectRows(tabPane) {
			...
			tabPane.querySelectorAll('.committee-tbody tr').forEach(function(tr) {
				...
				rows.push({
					...
					peranan: (tr.querySelector('.peranan-select') && tr.querySelector('.peranan-select')
						.value) || '3',
				});
			});
		...
		function buildDraftFormDataAll() {
			...
			document.querySelectorAll('.committee-pane').forEach(function(tabPane) {
				...
				const catatanValue = (tabPane.querySelector('.committee-catatan') && tabPane.querySelector(
					'.committee-catatan').value) || '';
		...
		function saveAllDrafts(triggerButton, successModal) {
			if (!tenderUuid) {
				alert(
					'{{ @$tender->procurement_label }} tidak ditemui. Sila buka semula halaman menggunakan pautan {{ strtolower(@$tender->procurement_label) }}.');
				return;
			}
