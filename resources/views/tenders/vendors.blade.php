					...
					<tr>
						...
						@if (count($tender->siteVisits()->orderBy('id', 'asc')->get()) > 0)
							@foreach ($tender->siteVisits()->get() as $visit)
								<td><input type="checkbox" class="visit-{{ $visit->id }}" name="visits[{{ $visit->id }}][]"
										value="{{ $purchase->vendor_id }}" @if (App\TenderVisitor::hasVisit($visit->id, $purchase->vendor_id)) checked @endif"></td>
							@endforeach
						@endif

						@if ($tender->canShowPrices())
							<td><input type="text" name="label[{{ $purchase->id }}]" value="{{ $purchase->label }}" class="form-control">
							</td>
							<td><input type="text" name="price[{{ $purchase->id }}]" value="{{ $purchase->price }}" class="form-control">
							</td>
							<td align="center">
								<input type="radio" name="winner" value="{{ $purchase->id }}"
									@if ($purchase->winner) checked @endif><br><br>
								<input type="text" name="project_timeline" value="{{ $purchase->winner ? $purchase->project_timeline : '' }}"
									@if (!$purchase->winner) disabled="disabled" @endif placeholder="Tempoh Siap" class="form-control">
							</td>
						@endif
						{{-- Check if there are winner for this tender yet. If yes, this column will appear --}}
						@if ($count_winner > 0)
