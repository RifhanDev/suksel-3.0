...
<div class="row stacked-form">
	<div class="col-lg-2">
		<ul class="nav nav-pills nav-stacked">
			<li role="presentation" class="active"><a href="#tf-main" aria-controls="home" role="tab" data-toggle="tab">Maklumat
					Tender</a></li>
			...
			<li role="presentation"><a href="#tf-khas" aria-controls="home" role="tab" data-toggle="tab">Syarat
					Khas</a></li>
			<li role="presentation"><a href="#tf-lawatan" aria-controls="messages" role="tab" data-toggle="tab">Lawatan
					Tapak</a></li>
	...
	<div class="tab-content col-lg-10">
		<div role="tabpanel" class="tab-pane active" id="tf-main">
				...
				<div class="col-md-12 my-2">
					<div class="row">
						<div class="col-md-2 d-flex justify-content-end">
							<label for="organization_unit_id" class="">Agensi <span class="text-danger">*</span></label>
						</div>
						<div class="col-md-10">
							<select class="form-control" name="organization_unit_id" id="organization_unit_id" required>
								<option value="">- Pilihan Agensi -</option>
								@foreach ($organizations as $_orgs)
									<option value="{{ $_orgs->id }}"
										{{ old('organization_unit_id', isset($tender) ? $tender->organization_unit_id : '') == $_orgs->id ? 'selected' : '' }}>
										{{ strtoupper($_orgs->name) }}
									</option>
								@endforeach
							</select>
						</div>
			...
			<div class="form-group">
				...
				<div class="col-lg-10 col-sm-10">
					<div class="input-group">
						<input class="form-control x-uppercase" id="advertise_start_date" type="text" name="advertise_start_date"
							required
						...
						<input class="form-control x-uppercase" id="advertise_stop_date" type="text" name="advertise_stop_date"
							required
			...
			<div class="form-group">
				...
				<div class="col-lg-10 col-sm-10">
					<div class="input-group">
						<input class="form-control x-uppercase" id="document_start_date" type="text" name="document_start_date"
							required
						...
						<div class="input-group-addon">hingga</div>
						<input class="form-control x-uppercase" id="document_stop_date" type="text" name="document_stop_date" required
			...
			<?php $submission_datetime = Former::text('submission_datetime')->label('Tarikh Tutup')->required(); ?>
			<?php if (isset($tender)) {
			...
			<?php $briefing_datetime = Former::text('briefing_datetime')->data_date_format('D MMM YYYY H:mm')->label('Tarikh & Masa Taklimat'); ?>
				...
				<div class="col-md-6">
					...
					<div class="form-group required">
						...
						<div class="col-lg-9 col-sm-9">
							<input type="text" class="form-control" id="default_name" value="{{ auth()->user()->name }}" readonly>
							@if (isset($tender->creator->id))
								<input type="hidden" class="form-control" id="default_creator_id" name="default_creator_id"
									value="{{ $tender->creator->id }}" readonly>
							@endif
						</div>
					</div>
					<div class="form-group required">
						...
						<div class="col-lg-9 col-sm-9">
							<input type="email" class="form-control" id="default_email" value="{{ auth()->user()->email }}" readonly>
						</div>
					</div>
					<div class="form-group required">
						...
						<div class="col-lg-9 col-sm-9">
							...
							@else
								<input type="text" class="form-control" id="default_tel" name="default_tel" required>
							@endif
						</div>
					</div>
					<div class="form-group required">
						...
						<div class="col-lg-9 col-sm-9">
							@if (isset(auth()->user()->department))
								<input type="text" class="form-control" id="default_department" name="default_department"
									value="{{ auth()->user()->department }}" required>
							@else
								<input type="text" class="form-control" id="default_department" name="default_department" required>
							@endif
						</div>
					</div>
				</div>
		...
		<div role="tabpanel" class="tab-pane" id="tf-khas">
				...
				<div class="col-lg-9 col-sm-9">
					<div class="checkbox">
						<input id="only_selangor1" type="checkbox" name="only_selangor" value="1"
							onclick="changeOnlySelangorCheckbox(this)" {{ ($tender->only_selangor ?? 0) == 1 ? 'checked' : '' }} required>
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				<label for="only_selangor2" class="control-label col-lg-3 col-sm-3">Syarikat Selangor <br>Dan Lain-lain
					Negeri</label>
				<div class="col-lg-9 col-sm-9">
					<div class="checkbox">
						<input id="only_selangor2" type="checkbox" name="only_selangor" value="2"
							onclick="changeOnlySelangorCheckbox(this)" {{ ($tender->only_selangor ?? 0) == 2 ? 'checked' : '' }} required>
					</div>
				</div>
			</div>
				...
				<div class="col-lg-9 col-sm-9">
					<div class="checkbox">
						<input id="only_selangor3" type="checkbox" name="only_selangor" value="3"
							onclick="changeOnlySelangorCheckbox(this)" {{ ($tender->only_selangor ?? 0) == 3 ? 'checked' : '' }} required>
					</div>
				</div>
			</div>
							...
							@foreach ($district_list_rule as $row_id => $district_rule)
								...
								<div id="tobe_cloned{{ $row_id }}" name="tobe_cloned"
									...
									@if ($district_rule->district_id == 0)
										<div class="col-lg-6 col-sm-6" id="district_id_div{{ $row_id }}" name="district_id_div">
											<select class="form-control district_select" id="district_id_new{{ $row_id }}"
										...
										<div id="state_id_div{{ $row_id }}" name="state_id_div" class="col-lg-5 col-sm-5 " style="">
											<select class="form-control state_select" id="state_id_new{{ $row_id }}" name="state_id_new[]"
												style="" {{ ($tender->only_selangor ?? 0) == 1 ? 'disabled' : '' }}>
												<option value="0" {{ $district_rule->state_id == 0 ? 'selected' : '' }}>Pilihan Negeri...
												</option>
												@foreach ($country_states as $state)
													<option value="{{ $state->id }}" {{ $state->id == $district_rule->state_id ? 'selected' : '' }}>
										...
										@endif
									@else
										<div class="col-lg-11 col-sm-11" id="district_id_div{{ $row_id }}" name="district_id_div">
										...
										<div id="state_id_div{{ $row_id }}" name="state_id_div" class="col-lg-5 col-sm-5 "
											style="display:none">
											<select class="form-control state_select" id="state_id_new{{ $row_id }}" name="state_id_new[]"
												style="display:none">
												<option value="0" {{ $district_rule->state_id == 0 ? 'selected' : '' }}>Pilihan Negeri...
			...
			<div class="form-group">
				...
				<div class="col-lg-10 col-sm-10">
					<div class="checkbox">
						<input type="checkbox" name="only_advertise" @if (isset($tender) && !empty($tender->only_advertise)) checked="checked" @endif
							value="1">
					...
					<div class="row r-b-10">
						...
						<div class="col-md-3">
							<input id="site_visit_form_#index#_datetime" class="sitevisit-date form-control"
								data-date-format="D MMM YYYY H:mm" name="site_visits[#index#][date]" type="text" />
						</div>
						<div class="col-md-2">
		...
		<div role="tabpanel" class="tab-pane" id="tf-kod">
			<div class="form-group">
				...
				<div class="col-lg-10 col-sm-10">
					<div id="mof_form">
						<div v-for="(code, index) in codes" v-bind:class="'qc-mof-code-form index-' + index" v-bind:data-index="index">
							<div class="row">
								<div class="col-lg-3">
									<select v-bind:id="'mof_form_' + index + '_inner_rule'" v-bind:name="'mof_codes[' + index + '][inner_rule]'"
								...
								<div class="col-lg-7">
									<select v-bind:id="'mof_form_' + index + '_codes'" v-bind:name="'mof_codes[' + index + '][codes][]'"
										class="mof-code-codes form-control selectize" multiple="multiple" v-model="code.codes">
								...
								<div class="col-lg-2">
									<a class="btn btn-warning btn-raised" v-show="codes.length > 1" v-on:click="deleteForm(index)">&times;</a>
								</div>
							</div>

							<div class="row join-rule" v-show="codes.length > 1 && index < (codes.length - 1)">
								<div class="col-lg-3 col-lg-offset-3">
									<select v-bind:id="'mof_form_' + index + '_join_rule'" v-bind:name="'mof_codes[' + index + '][join_rule]'"
					...
					<div id="cidb_form">
						<div v-for="(code, index) in codes" v-bind:class="'qc-cidb-code-form index-' + index"
							...
							<div class="row">
								<div class="col-lg-3">
									<select v-bind:id="'cidb_form_' + index + '_inner_rule'"
										v-bind:name="'cidb_codes[' + index + '][inner_rule]'" v-model="code.inner_rule"
										class="cidb-codes-inner-rule form-control">
										<option value="and">Dan</option>
										<option value="or">Atau</option>
									</select>
								</div>
								...
								<div class="col-lg-7">
									<select v-bind:id="'cidb_form_' + index + '_codes'" v-bind:name="'cidb_codes[' + index + '][codes][]'"
										class="cidb-codes-codes form-control selectize" multiple="multiple" v-model="code.codes">
							...
							<div class="row join-rule">
								<div class="col-lg-3 col-lg-offset-3" v-show="codes.length > 1 && index < (codes.length - 1)">
									<select v-bind:id="'cidb_form_' + index + '_join_rule'" v-bind:name="'cidb_codes[' + index + '][join_rule]'"
										v-model="code.join_rule" class="cidb-codes-join-rule form-control">
										<option value="and">Dan</option>
										<option value="or">Atau</option>
									</select>
								</div>
		...
		<div role="tabpanel" class="tab-pane" id="tf-doc1">
			...
			<div id="files">
				<div id="files_template">
					<div class="row">
						<div class="col-lg-4">
							<input type="file" id="files_file_#index#" name="files[#index#][file]" class="form-control">
						</div>

						<div class="col-lg-6">
							<input type="text" id="files_name_#index#" name="files[#index#][name]" placeholder="Nama Dokumen"
								class="form-control name">
							<label>
								<input type="checkbox" id="files_public_#index#" name="files[#index#][public]" class="doc-2"> Dokumen Meja
								Terkawal
							</label>
						</div>
