	...
	<div class="news-huge">
		...
		<div class="well">
			...
			@if ($news->canUpdate())
				...
				@if (!$news->publish)
					<a href="{{ asset('news/' . $news->id . '/publish') }}" class="btn btn-warning link-confirm">Siar</a>
				@endif
				@if ($news->publish)
					<a href="{{ asset('news/' . $news->id . '/publish') }}" class="btn btn-danger link-confirm">Batal Siar</a>
				@endif
