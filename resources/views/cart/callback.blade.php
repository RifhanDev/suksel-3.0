			...
			@forelse($tenders as $tender)
				<tr>
					...
					@if ($transaction->status == 'success')
						<td>
							<a
								href="{{ asset('tenders/' .$tender->id .'/receipt/' .$tender->participants()->whereVendorId(Auth::user()->vendor_id)->first()->id) }}"
								target="_blank"><i class="icon-printer"> Resit</i></a><br>
							<a
								href="{{ asset('tenders/' .$tender->id .'/document/' .$tender->participants()->whereVendorId(Auth::user()->vendor_id)->first()->id) }}"
								target="_blank"><i class="icon-doc"> No. Siri Dokumen</i></a>
						</td>
					@endif
				</tr>
			@empty
				<tr>
					<td colspan="5">
						<center>Tiada tender dalam senarai tempahan</center>
					</td>
				</tr>
			@endforelse
		</tbody>
		<tfoot>
			...
			@if ($transaction->status == 'success')
				<tr>
					...
					<td @if ($transaction->status == 'success') colspan="2" @endif>
						{{ $transaction->vendor_id }}-{{ $transaction->gateway_reference }}</td>
				</tr>
			@endif
			<tr>
			...
			<div id="payment-options" class="pull-right">
				...
				<div class="btn-toolbar">
					@if ($amount > 0.0)
						@if ($fpx)
							<div class="btn-group">
								<a href="#" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">Internet Banking (FPX) <span
										class="caret"></span></a>
