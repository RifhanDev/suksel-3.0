...
@section('content')
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="{{ action('OrganizationUnitsController@prices', $organizationunit->id) }}">Carta Tender</a>
		</li>
		...
		<div class="col-md-2">
			<ul class="nav nav-pills nav-stacked">
				<li @if (!Request::get('type')) class="active" @endif><a
						href="{{ action('OrganizationUnitsController@prices', $organizationunit->id) }}" role="tab">Semua</a></li>
				<li @if (Request::get('type') == 'tenders') class="active" @endif><a
						href="{{ action('OrganizationUnitsController@prices', [$organizationunit->id, 'type' => 'tenders']) }}">Tender</a>
				</li>
				<li @if (Request::get('type') == 'quotations') class="active" @endif><a
						href="{{ action('OrganizationUnitsController@prices', [$organizationunit->id, 'type' => 'quotations']) }}">Sebut
						Harga</a></li>
			</ul>
		</div>
								...
								<td>
									<div class="btn-group btn-group-vertical">
										@if (empty($tender->approver_id))
											<a class="btn btn-primary btn-xs" href="{{ action('tenders.edit', $tender->id) }}">Kemaskini</a>
										@endif
