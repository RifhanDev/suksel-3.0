...
@section('content')
		...
		@if (Auth::check())
			<div class="btn-group pull-right">
				@if (Auth::user()->hasRole('Admin'))
					<a href="{{ route('agencies.edit', $organizationunit->id) }}" class="btn btn-warning btn-sm"><i
							class="fa fa-pencil"></i> Kemaskini Agensi</a>
				@endif
				@if (Auth::user()->ability(['Admin', 'Agency Admin', 'Agency User'], []))
					<a href="{{ route('tenders.create') }}" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> Tambah Tender / Sebut
						Harga</a>
				@endif
			</div>
		@endif
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="{{ action('OrganizationUnitsController@results', $organizationunit->id) }}">Penender
				Berjaya</a></li>
		...
		<div class="col-md-2">
			<ul class="nav nav-pills nav-stacked">
				<li @if (!Request::get('type')) class="active" @endif><a
						href="{{ action('OrganizationUnitsController@results', $organizationunit->id) }}" role="tab">Semua</a></li>
				<li @if (Request::get('type') == 'tenders') class="active" @endif><a
						href="{{ action('OrganizationUnitsController@results', [$organizationunit->id, 'type' => 'tenders']) }}">Tender</a>
				</li>
				<li @if (Request::get('type') == 'quotations') class="active" @endif><a
						href="{{ action('OrganizationUnitsController@results', [$organizationunit->id, 'type' => 'quotations']) }}">Sebut
						Harga</a></li>
			</ul>
		</div>
