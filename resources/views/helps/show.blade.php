			...
			<h1 class="tender-title">
				...
				@if (Auth::user() && Auth::user()->hasRole('Admin'))
					<div class="btn-group pull-right">
						...
						<a href="{{ asset('helps/create') }}" class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> Tambah Soalan
							Lazim</a>
					</div>
				@endif
			</h1>

			<h4 class="pull-left">{{ $category->name }} <small>({{ $helps->count() }} artikel) <br>
					{{ $category->description }}</small></h4>
