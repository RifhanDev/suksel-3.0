...
@section('content')
	...
	<form id="saveForm" name="saveForm" action="{{ route('chatbot-manager.category.update', ['category' => $data->enc_id]) }}"
			...
			<div class="col-lg-9 col-sm-9">
				<div id="show_none_btn_div" class="form-group form-check pl-4">
					<input type="checkbox" class="form-check-input" id="show_none_btn" name="show_none_btn" value="1"
						{{ ($data->show_none_btn ?? 0) == '1' ? 'checked' : '' }}>
					<label class="form-check-label" for="show_none_btn">Papar butang <q>Bukan disenarai diatas</q></label>
				</div>

				<span for="show_none_btn_div">**Nota: Papar pilihan tambahan jika soalan yang ingin ditanya tidak wujud di pangkalan
					data</span>
			</div>
		</div>
		...
		<div class="well">
			...
			<a href="{{ route('chatbot-manager.category.index') }}" class="btn btn-default pull-right">Senarai Kategori Soalan
				ChatBot</a>
		</div>
	</form>
