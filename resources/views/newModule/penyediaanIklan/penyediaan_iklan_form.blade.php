@push('styles')
	...
	<style>
		...
		.iklan-stepper-wrapper[data-step="1"] .iklan-stepper-progress {
			width: 0%;
		}

		.iklan-stepper-wrapper[data-step="2"] .iklan-stepper-progress {
			width: 50%;
			background: var(--sg-red, #c41e3a);
		}

		.iklan-stepper-wrapper[data-step="3"] .iklan-stepper-progress {
			width: 100%;
			background: linear-gradient(90deg, #10b981 0%, #10b981 10%, var(--sg-red, #c41e3a) 100%);
		}
		...
		.iklan-step-item.active .iklan-step-name {
		...
		.iklan-step-item.completed .iklan-step-counter::after {
			content: '✓';
			font-size: 0.9rem;
		}

		.iklan-step-item.completed .iklan-step-counter span {
			display: none;
		}

		.iklan-step-item.completed .iklan-step-name {
			color: #10b981;
		}
		...
		.kebenaran-check-card .kc-label {
			font-size: 0.85rem;
			font-weight: 600;
			color: #1e293b;
		}

		.kebenaran-check-card .kc-desc {
			font-size: 0.75rem;
			color: #64748b;
			margin-top: 2px;
		}
		...
		.sk-radio-card:has(input:checked) {
			border-color: var(--sg-red, #c41e3a) !important;
			background: #fef2f2;
		}

		.sk-radio-card input[type=radio] {
			width: 16px;
			height: 16px;
			accent-color: var(--sg-red, #c41e3a);
			flex-shrink: 0;
		...
		.sk-check-row:has(input:checked) {
			border-color: var(--sg-red, #c41e3a);
			background: #fef2f2;
		}

		.sk-check-row input[type=checkbox] {
			width: 16px;
			height: 16px;
			accent-color: var(--sg-red, #c41e3a);
			flex-shrink: 0;
		...
		.sk-atau-divider::before,
		.sk-atau-divider::after {
			content: '';
			flex: 1;
			height: 1px;
			background: #e2e8f0;
...
<div class="content-card mb-4 p-0">
	...
	@php
		...
		$invitation = array_key_exists('invitation', $syarat) ? $syarat['invitation'] : (bool) ($tender->invitation ?? false);
				...
				<div class="col-lg-8">
					...
					<div class="d-flex flex-column gap-2">
						<label class="sk-radio-card d-flex align-items-center gap-3 px-3 py-2 border rounded-2">
							<input type="radio" name="only_selangor" id="only_selangor1" value="1" @checked((string) $onlySelangor === '1')>
							<span class="sk-radio-label">Syarikat Selangor Sahaja</span>
						</label>
						<label class="sk-radio-card d-flex align-items-center gap-3 px-3 py-2 border rounded-2">
							<input type="radio" name="only_selangor" id="only_selangor2" value="2" @checked((string) $onlySelangor === '2')>
							<span class="sk-radio-label">Syarikat Selangor Dan Lain-lain Negeri</span>
						</label>
						<label class="sk-radio-card d-flex align-items-center gap-3 px-3 py-2 border rounded-2">
							<input type="radio" name="only_selangor" id="only_selangor3" value="3" @checked((string) $onlySelangor === '3')>
				...
				<div class="col-lg-4">
					...
					<div class="d-flex flex-column gap-2">
						<label class="sk-check-row">
							<input type="checkbox" name="only_bumiputera" id="sk_bumi" value="1" @checked($isChecked($onlyBumiputera))>
							<span class="sk-check-label">Bumiputera Sahaja</span>
						</label>
						<label class="sk-check-row">
							<input type="checkbox" name="invitation" id="sk_terhad" value="1" @checked($isChecked($invitation))>
							<span class="sk-check-label">Tender Terhad</span>
						</label>
						<label class="sk-check-row">
							<input type="checkbox" name="only_advertise" id="sk_iklan" value="1" @checked($isChecked($onlyAdvertise))>
							<span class="sk-check-label">Iklan Sahaja</span>
						</label>
					</div>

					<div class="d-flex align-items-start gap-2 mt-3 p-3 rounded-2"
						style="font-size:0.76rem; color:#0369a1; line-height:1.5; background:#eff6ff; border:1px solid #bae6fd;">
				...
				<div class="modal-body p-4">
					<div class="row g-3">
						...
						<div class="col-12">
							...
							<textarea class="form-control" id="taklimat_perihal" name="taklimat_perihal" rows="2"
							 placeholder="Huraikan perihal taklimat..."></textarea>
						</div>

						{{-- Row 2: Tarikh | Masa | Kehadiran --}}
						<div class="col-md-4">
							<label class="form-label fw-semibold">Tarikh <span class="text-danger">*</span></label>
							<input type="text" class="form-control form-control-lg datepicker" id="taklimat_tarikh"
						...
						<div class="col-12">
							...
							<textarea class="form-control" id="taklimat_lokasi" name="taklimat_lokasi" rows="2"
							 placeholder="Masukkan lokasi atau alamat penuh..."></textarea>
						</div>
	...
	<script>
		$(document).ready(function() {
			...
			function iklanGoToStep(step) {
				...
				$.each(iklanStepContents, function(i, sel) {
					$(sel).hide();
				});
			...
			function savePenyediaanIklanDraft(done) {
				...
				$.ajax({
					...
					complete: function() {
						var includeMejaInSave = mejaStepVisited || (savedMejaDocs && savedMejaDocs.length >
							0);
						$('#mejaTerkawalRows').find(':input').prop('disabled', !includeMejaInSave);
			...
			$('#iklanBtnBack2').on('click', function() {
				iklanGoToStep(1);
			});
			...
			$('#iklanBtnBack3').on('click', function() {
				iklanGoToStep(2);
			});
			...
			savedTaklimat.forEach(function(row) {
				appendTaklimatRow(row);
			});
			...
			$('#btnHapusTaklimat').on('click', function() {
				$('#tblTaklimatBody .taklimat-main-row').each(function() {
					if ($(this).find('.chk-taklimat-row').is(':checked')) {
						...
						taklimatRows = taklimatRows.filter(function(r) {
							return r.rid !== rId;
						});
						$('#detail_' + rId).remove();
						$(this).remove();
			...
			$('#btnTambahDaerah').on('click', function() {
				...
				var row = [
					...
					'<select class="form-select form-select-sm district_select" id="district_id_new' +
					idx + '" name="district_id_new[]">',
					skBaseDaerahOpts + luarOpt,
					'</select>',
					'</div>',
					'<div id="state_id_div' + idx + '" style="display:none;min-width:180px;">',
					'<select class="form-select form-select-sm" id="state_id_new' + idx +
					'" name="state_id_new[]">',
					skNegeriOpts,
					'</select>',
					'</div>',
			...
			function restoreSyaratKhas() {
				if (savedOnlySelangor) {
					$('[name="only_selangor"][value="' + savedOnlySelangor + '"]').prop('checked', true).trigger(
						'change');
				}
			...
			function buildMejaTerkawalRow(doc, index) {
				...
				var existingNote = existingPath ?
					...
					(existingUrl ? ' · <a href="' + escapeMejaAttr(existingUrl) +
				...
				return (
					...
					'<input type="text" class="form-control form-control-sm" name="dokumen_meja[' + i +
					'][nama]" value="' + escapeMejaAttr(displayName) +
					'" placeholder="cth: Spesifikasi Teknikal">' +
					(existingPath ? '<input type="hidden" name="dokumen_meja[' + i + '][existing_path]" value="' +
						escapeMejaAttr(existingPath) + '">' : '') +
					(uploadId ? '<input type="hidden" name="dokumen_meja[' + i + '][upload_id]" value="' +
						escapeMejaAttr(uploadId) + '">' : '') +
					'</div>' +
					'<div class="col-md-5">' +
					'<label class="form-label fw-semibold small mb-1">Fail</label>' +
					'<input type="file" class="form-control form-control-sm" name="dokumen_meja[' + i +
					'][file]" accept=".pdf,.doc,.docx,.xls,.xlsx,.png,.jpg,.jpeg,.zip,.rar">' +
					existingNote +
					'</div>' +
