...
@section('content')
    @php
        $p = $project ?? $p ?? null;
        $decision = $decision ?? (object) ['company' => '-', 'harga_sst' => 0, 'status' => 'pending'];
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#btn-terima').on('click', function() {
                var form = $('<form>', { method: 'POST', action: '{{ route('pembelianTerus.storeKeputusan', $p->id) }}' });
                form.append($('<input>', { type: 'hidden', name: '_token', value: '{{ csrf_token() }}' }));
                form.append($('<input>', { type: 'hidden', name: 'decision', value: 'accepted' }));
                $('body').append(form);
                form.submit();
            });
            ...
            $('#btn-tolak').on('click', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var form = $('<form>', { method: 'POST', action: '{{ route('pembelianTerus.storeKeputusan', $p->id) }}' });
                        form.append($('<input>', { type: 'hidden', name: '_token', value: '{{ csrf_token() }}' }));
                        form.append($('<input>', { type: 'hidden', name: 'decision', value: 'rejected' }));
                        $('body').append(form);
                        form.submit();
                    }
                });
            ...
            $surat.on('click', function() {
                window.location.href = '{{ route('pembelianTerus.downloadSuratSetujuTerima', $p->id) }}';
            });
