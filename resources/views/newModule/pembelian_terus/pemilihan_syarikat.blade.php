    ...
    <div class="card-body p-4">
        ...
        <nav aria-label="breadcrumb" class="py-2 mb-3">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Pemilihan Syarikat</li>
            </ol>
        </nav>

        {{-- Page Title --}}
        <h4 class="fw-bold mb-4 pb-2 border-bottom" style="color: var(--sg-red-dark);">SENARAI PROJEK UNTUK PEMILIHAN SYARIKAT</h4>
        ...
        <div class="d-flex flex-column flex-md-row gap-3 align-items-end mb-4">
            <div class="w-100 flex-md-fill" style="min-width: 200px;">
                ...
                <select class="form-select" id="searchTenderNo" onchange="performLiveSearch()">
                    <option value="">Semua</option>
                    <option value="QT210000000023741">QT210000000023741</option>
                    <option value="QT210000000023740">QT210000000023740</option>
                </select>
            </div>

            <div class="w-100 flex-md-fill" style="min-width: 200px;">
                <label class="form-label fw-bold">Tajuk Perolehan</label>
                <select class="form-select" id="searchTajuk" onchange="performLiveSearch()">
                    <option value="">Semua</option>
                    <option value="BEKALAN BARANGAN PERSEKOLAHAN">BEKALAN BARANGAN PERSEKOLAHAN</option>
                    <option value="TAJUK PEROLEHAN 1">TAJUK PEROLEHAN 1</option>
                </select>
            </div>
            ...
            <div class="d-flex align-items-end w-100 w-md-auto">
                <button type="button" class="btn btn-tapis w-100 w-md-auto px-4" onclick="clearAllFilters()">Tapis</button>
            </div>
        </div>
                ...
                <table class="table project-table mb-0">
                    ...
                    <tbody id="projectTableBody">
                        <tr data-tender="QT210000000023741" data-tajuk="BEKALAN BARANGAN PERSEKOLAHAN" data-tarikh="3/3/2024">
                            <td>
                                <a class="text-decoration-none" href="{{ route('pembelianTerus.pemilihanSyarikatDetails', 'QT210000000023741') }}">
                        ...
                        <tr data-tender="QT210000000023740" data-tajuk="TAJUK PEROLEHAN 1" data-tarikh="2/2/2024">
                            <td>
                                <a class="text-decoration-none" href="{{ route('pembelianTerus.pemilihanSyarikatDetails', 'QT210000000023740') }}">
...
<script>
    ...
    function normalizeDate(dateString) {
        if (!dateString) return '';
        // Remove leading zeros from day and month for comparison
        // Convert "03/03/2024" to "3/3/2024" to match table format
    ...
    function filterTable() {
        const tenderNo = document.getElementById('searchTenderNo').value;
        const tajuk = document.getElementById('searchTajuk').value;
        ...
        rows.forEach(row => {
            const matchesTender = !tenderNo || row.getAttribute('data-tender') === tenderNo;
            const matchesTajuk = !tajuk || row.getAttribute('data-tajuk') === tajuk;
            
            // Normalize both dates for comparison (remove leading zeros)
            const formattedInputDate = formatDate(tarikhInput);
            const normalizedInputDate = normalizeDate(formattedInputDate);
            const normalizedTableDate = normalizeDate(row.getAttribute('data-tarikh'));
            const matchesTarikh = !tarikhInput || normalizedInputDate === normalizedTableDate;
