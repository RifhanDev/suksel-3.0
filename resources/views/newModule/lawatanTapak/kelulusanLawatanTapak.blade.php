    ...
    <div class="content-card mb-4 p-0">
        <div class="content-card-header p-4 pb-3 border-bottom">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    <h3 class="content-card-title mb-0" style="font-size: 1rem;">Kelulusan Lain-Lain</h3>
                    <p class="text-muted mb-0" style="font-size: 0.78rem;">Kemaskini butiran dan dokumen berkaitan untuk tujuan kelulusan lawatan tapak</p>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const tbody = document.querySelector("#dt_tmpltSpec tbody");
        const addBtn = document.querySelector('[data-bs-target="#tambahKehadiranModal"]');

        function buildItemRow(data = {}, index = "") {
            return `
                <tr class="item-row">
                    <td class="text-center row-no">${index}</td>
                    <td>
                        <input type="text" name="jenis_kelulusan[]" class="form-control form-control-sm" value="${data.jenis || ''}" placeholder="Jenis Kelulusan">
                    </td>
                    <td>
                        <input type="text" name="status[]" class="form-control form-control-sm" value="${data.status || ''}" placeholder="Status">
                    </td>
                    <td>
                        <input type="file" name="dokumen[]" class="form-control form-control-sm">
                    </td>
                    <td>
                        <input type="text" name="catatan[]" class="form-control form-control-sm" value="${data.catatan || ''}" placeholder="Catatan">
                    </td>
                    <td class="text-center">
                        <button type="button" class="btn btn-sm  btn-hapus-item d-inline-flex align-items-center justify-content-center p-0" style="width:28px;height:28px;border-radius:6px;background:#fee2e2;color:#ef4444;border:none;">
        ...
        function renumberRows() {
            tbody.querySelectorAll("tr.item-row").forEach((row, i) => {
                const numCell = row.querySelector(".row-no");
        ...
        function addRow(data = {}) {
            ...
            const index = tbody.querySelectorAll("tr.item-row").length + 1;

            tbody.insertAdjacentHTML("beforeend", buildItemRow(data, index));

            renumberRows();
        }

        // =========================
        // DEFAULT 2 FILLED ROWS
        // =========================
        const dummyData = [
            {
                jenis: "Kelulusan Teknikal",
                status: "Lulus",
                catatan: "Tiada isu"
            },
            {
        ...
        // if empty → show "Tiada Data"
        if (!tbody.querySelector("tr.item-row")) {
            tbody.innerHTML = `
                <tr id="tbl-no-data">
    ...
    (function () {
        ...
        if (btnClose) {
            btnClose.addEventListener('click', function () {
                ...
                setTimeout(() => {
                    window.location.href = "{{ route('lawatanTapakUrusetia') }}";
                }, 300);
            });
        }
